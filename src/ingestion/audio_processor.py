...
try:
    ...
    logger.error("Security modülleri bulunamadı. Lütfen dosya yapısını kontrol edin.")
...
class AudioProcessor:
    ...
    def __init__(self, db_session, model_size: str = Config.WHISPER_MODEL_SIZE):
        ...
        try:
            ...
            self.privacy = None
            logger.info("PrivacyManager sınırlı modda başlatıldı (DB Bağlantısı Yok).")
 
    def load_model(self):
        """Whisper modelini belleğe yükle (Lazy Loading)."""
        if self.model is None:
            logger.info(f"Whisper modeli yükleniyor: {self.model_size} ({self.device})...")
            self.model = whisper.load_model(self.model_size, device=self.device)
   
    def is_supported_format(self, audio_path: Path) -> bool:
        """Dosya formatının desteklenip desteklenmediğini kontrol eder."""
        supported_extensions = Config.SUPPORTED_AUDIO_FORMATS
    ...
    def transcribe_audio(self, audio_path: Path, item_id: int) -> Optional[str]:
        ...
        if not self.is_supported_format(audio_path):
            logger.warning(f"Desteklenmeyen format: {audio_path.suffix}")
            return None
        ...
        try:
            ...
            logger.error(f"Transkripsiyon hatası ({audio_path.name}): {e}")
            return None
