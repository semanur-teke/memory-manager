...
class TimelinePage:
    ...
    def get_timeline_data(
        ...
        Adimlar:
        1. Event'leri DB'den tarih sirasina gore al
        2. Her event icin ozeti decrypt_string() ile coz
        3. Kapak fotografisini decrypt_file() ile gecici olarak coz
    ...
    def decrypt_cover_photo(self, event_id: int) -> Optional[str]:
        ...
        if cover_item is None:
            logger.warning("Consentli kapak fotografi bulunamadi | event_id=%s", event_id)
            return None
    ...
    def group_events_by_period(
        ...
        for event in events:
            ...
            if period == "day":
                ...
                raise ValueError("period sadece 'day', 'week', 'month' veya 'year' olabilir.")
    ...
    def get_timeline_stats(self) -> Dict:
        ...
        distinct_locations = {
            location
            for (location,) in self.db.query(Event.main_location).all()
            if location
        }

        date_range = (
            base_stats.get("earliest_date"),
            base_stats.get("latest_date"),
        )
